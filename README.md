# RX3 on Orange Pi 4 LTS with DDJ-400

Run the **embedded XDJ-RX3 1.19 player** with compatibility helpers on an Orange Pi 4 LTS (RK3399), a 1920x1080 landscape display and a Pioneer DDJ-400. This is experimental software, not desktop rekordbox and not firmware you flash onto the board.
...
- Orange Pi 4 LTS; 64-bit Debian/Armbian with Python 3.9+, 4 KiB kernel pages and ARM32 compatibility (`CONFIG_COMPAT`). `doctor` checks execution rather than assuming compatibility from the board name.
- The display code in this branch supports **1920×1080 landscape**. The 1280×800 RX3 image is presented as 1728×1080 with 96-pixel pillarbox bars. Other display sizes need code changes.
- DDJ-400 audio/MIDI. The intended route is master on channels 1/2 and headphones on 3/4 at 44.1 kHz; confirm the ALSA card ID and four-channel playback on the physical controller.
...
Clone this integrated branch:

```sh
git clone --branch orangepi4-ddj400-integrated https://github.com/fabiodj2/RX3-Orange-PI-4-LTS.git
...
Setup downloads about 322 MB of official ZIPs, recovers the key privately, decrypts/verifies the firmware, creates the complete runtime, compiles the helpers and installs the matching player/shim. It also obtains a checksum-pinned official Mixxx DDJ-400 XML mapping if the configured file is missing. Existing mapping files are preserved and checked. Mixxx itself is not required.
...
For an existing DDJ-400 mapping, set `[controller] mapping` to its XML path and run `./rx3 mapping --offline`. This bridge translates supported XML bindings; it does not execute the XML's referenced Mixxx JavaScript.
...
| No display / unsupported geometry | `./rx3 doctor`; this branch requires a connected 1920×1080 DRM mode. It can run directly through DRM even when the host has no `/dev/fb0`. Configure the DRM card only if auto-detection is ambiguous. |
...
| `sched_setscheduler(... SCHED_RR ...) = EPERM`, followed by `UniNo=0x0000000d` or `SIGSEGV` | Use this integrated launcher. It invokes the player through `prlimit --rtprio=95 --memlock=unlimited` before `chroot --userspec`; editing `limits.conf` alone is not reliable for this launch path. |
...
The upstream development installation demonstrated browsing/loading, playback through FLX6 headphones, stacked waveforms and several touch/mixer controls. The Orange Pi 4 LTS display path and DDJ-400 mapping still require physical acceptance testing. This work does not claim all FX/pad/LED behavior or perfect touchscreen parity.
