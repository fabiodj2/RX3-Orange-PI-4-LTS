...
# Orange Pi 4 LTS port: 1920x1080 landscape (was 1200x1920 portrait, the
# 10-inch Raspberry Pi Touch Display 2). See DISPLAY-PORT-NOTES.md.
PANEL = (1920, 1080)
...
DEFAULTS = {
    ...
    'controller': {
        'midi_name': 'DDJ-400',
        'mapping': 'work/Pioneer-DDJ-400.midi.xml',
    },
    'usb': {
        'uuid': '',
    },
...
class Config:
    ...
    def drm_device(self):
        ...
        if len(panel) > 1:
            raise Failure(f'More than one connected {PANEL[0]}x{PANEL[1]} display found; set [display] drm_device',
        ...
        if connected:
            ...
            raise Failure(f'No connected display reports the supported {PANEL[0]}x{PANEL[1]} mode',
                          f'Found {modes}. This checkout is pinned to a {PANEL[0]}x{PANEL[1]} landscape panel - see DISPLAY-PORT-NOTES.md.')
